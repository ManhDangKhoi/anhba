...
xpcall(function()
    if not isfolder("scripts/ID/" .. game.GameId) then
        if syn then
            makefolder("scripts/ID/" .. game.GameId)
            writefile("scripts/ID/" .. game.GameId .. "/" .. game.Players.LocalPlayer.Name .. ".lua",
                game:GetService("HttpService"):JSONEncode(SettingsGameData));
        else
            makefolder("scripts")
            makefolder("scripts/ID")
            makefolder(" scripts/ID/" .. game.GameId)
            writefile("scripts/ID/" .. game.GameId .. "/" .. game.Players.LocalPlayer.Name .. ".lua",
                game:GetService("HttpService"):JSONEncode(SettingsGameData));
        end
    ...
    SettingsGameData = game:GetService("HttpService"):JSONDecode(readfile(
        "scripts/ID/" .. game.GameId .. "/" .. game.Players.LocalPlayer.Name .. ".lua"));
end, function()
end)
-- fix lag
...
local Window = Luxury:CreateWindow({
    Title = "Peguin Hub | Premium",
    SubTitle = "HEHE",
    TabWidth = 160,
    Size = UDim2.fromOffset(580, 460),
    Acrylic = false, --Ko cần biết
    Theme = "Darker",
    MinimizeKey = Enum.KeyCode.LeftControl --Ko cần key
})
local Tabs = {
    infor = Window:AddTab({
        Title = "Infor"
...
Tabs.Main:AddParagraph({
    Title = "Settings",
    Content = "Select Weapons For Use Auto Find Items "
})
...
SelectedFastAttackMode = {"Normal Attack", "Fast Attack", "Cách cụt đấm nhau"}
...
local Boss_Dropdown = Tabs.Main:AddDropdown({
...
function WebHookLog:WebHookKaiTanSend(WebHookUrl)
    ...
    if _G.SendWebHookPing then
        Message = {
            ['username'] = "peguinHub Webhook",
            ["avatar_url"] = "https://cdn.discordapp.com/avatars/1180382217424347197/4175a218f279c4382a0e989592a87082.png?size=4096",
            ["content"] = "@everyone",
            ["embeds"] = Embeds
        }
    else
